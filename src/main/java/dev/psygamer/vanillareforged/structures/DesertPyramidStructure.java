...
public class DesertPyramidStructure extends Structure<NoFeatureConfig> {
	...
	public List<MobSpawnInfo.Spawners> getDefaultSpawnList() {
		return DesertPyramidStructure.STRUCTURE_MONSTERS;
	}
	...
	public List<MobSpawnInfo.Spawners> getDefaultCreatureSpawnList() {
		return DesertPyramidStructure.STRUCTURE_CREATURES;
	}
	...
	protected boolean isFeatureChunk(final ChunkGenerator chunkGenerator, final BiomeProvider biomeSource, final long seed, final SharedSeedRandom chunkRandom, final int chunkX, final int chunkZ, final Biome biome, final ChunkPos chunkPos, final NoFeatureConfig featureConfig) {
		final BlockPos centerOfChunk = new BlockPos((chunkX << 4) + 7, 0, (chunkZ << 4) + 7);
		
		// Grab height of land. Will stop at first non-air block.
		final int landHeight = chunkGenerator.getFirstOccupiedHeight(centerOfChunk.getX(), centerOfChunk.getZ(), Heightmap.Type.WORLD_SURFACE_WG);
		
		// Grabs column of blocks at given position. In overworld, this column will be made of stone, water, and air.
		// In nether, it will be netherrack, lava, and air. End will only be endstone and air. It depends on what block
		// the chunk generator will place for that dimension.
		final IBlockReader columnOfBlocks = chunkGenerator.getBaseColumn(centerOfChunk.getX(), centerOfChunk.getZ());
		
		// Combine the column of blocks with land height and you get the top block itself which you can test.
		final BlockState topBlock = columnOfBlocks.getBlockState(centerOfChunk.above(landHeight));
		...
		public void generatePieces(final DynamicRegistries dynamicRegistryManager, final ChunkGenerator chunkGenerator, final TemplateManager templateManagerIn, final int chunkX, final int chunkZ, final Biome biomeIn, final NoFeatureConfig config) {
			...
			JigsawManager.addPieces(
					...
					new VillageConfig(() -> dynamicRegistryManager.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
							...
							.get(new ResourceLocation(VanillaReforged.MODID, "run_down_house/start_pool")),
			...
			VanillaReforged.LOGGER.log(Level.DEBUG, "Rundown House at " +
